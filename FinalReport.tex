...
Notes: \\
\par \text{The eigenvector calculation is done by the power iteration method and has no guarantee of convergence. The iteration will stop after max\_iter iterations or an error tolerance of number\_of\_nodes(G)\*tol has been reached.} \cite{pr_algo1} \cite{pr_algo2}\\
...
\item Create an independent copy of $W^-$, denoted by $\hat{W}^-$
\item Generate $W^+$ by
$$W^+ = ad(W^-)^S+a(1-d)(W^-)^s$$
\item Independently generate $(D^+, D^-) = (Poisson(W^+), Poisson(W^-))$. Apply the sequence-modification algorithm in \cite{algo} (a brief description in \hyperlink{equal_sum_algo}{sequence-modification algorithm in Appendix} ) to make the sum of in-degree sequence equal to the sum of out-degree sequence.
\end{steps}  
...
Case1($\alpha=\beta$): Page Rank and Betweenness Centrality values of First 200 nodes \textbf{in decreasing order of Betweenness Centrality} with increasing d values from 0 to 1}
\end{figure}
...
\begin{figure}
\includegraphics[scale=0.8]{final_images/corrs_relation.png}
...
Case2 ($\alpha \neq \beta$): Page Rank and Betweenness Centrality values of First 200 nodes \textbf{in decreasing order of Betweenness Centrality} with increasing d values from 0 to 1}
\end{figure}
...
\hfill
\subfigure[corr = 1(perfectly correlated, in degree sequence = out degree sequence)]{\includegraphics[width=8cm]{final_images/avgsam.png}}
\hfill
\caption{
\label{comps}%
...
\caption{\label{wikivote_fit} log-log plot of tail distribution of Wiki-vote network}
\end{figure}

\item Node Elimination Effect on Wiki-vote Network\\

\par We consecutively eliminate 200 nodes from Wiki-vote network. The node elimination effect on the graph's average shortest path length is shown in Figure \ref{wikivote_elim}, and the output data is \href{https://github.com/leahwu/DCM_simulaiton_and_analysis/tree/master/output_data/wiki_vote}{here}.

\begin{figure}[!htbp]
\centering\includegraphics[width=10cm, height=8cm]{final_images/wikivote.png}
\caption{\label{wikivote_elim} Node elimination effect on ASPL of Wiki-vote network.}
\end{figure}
...
\bibitem{bc_algo1}
\text{Ulrik Brandes: A Faster Algorithm for Betweenness Centrality.} Journal of Mathematical Sociology \text{25(2):163-177, 2001.}
